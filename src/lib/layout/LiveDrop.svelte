...
<aside onmouseleave={outHover} class='hidden md:flex fixed left-0 top-[68px] w-[190px] flex-col gap-[1px]'>
    {#each drops as { id, rarity, itemImg, itemName, fromName, fromLink, fromImg, steamLink, steamImg }, index (id)}
        <div class='relative'>
            ...
            class='{hover != index 
                ...
                <a href={fromLink} class='col-span-2 row-span-3 col-start-1 row-start-2'><img src={fromImg} alt='from img' class='h-full object-contain'></a>
                <a href={steamLink} class='row-span-2 col-start-3 row-start-1'><img src={steamImg} alt='steam img' class='h-full rounded-full'></a>
                <img src={itemImg} alt='item img' class='row-span-2 col-start-3 row-start-3 h-full object-contain'>
            </div>
        </div>
    {/each}
</aside>
