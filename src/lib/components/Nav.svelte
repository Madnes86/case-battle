...
<nav class="{classP} {mobile ? 'w-full flex flex-col gap-2' : 'hidden md:flex h-full'}">
    {#each navs as nav}
        <a 
        href="/{nav}"
        class="group {$active == nav ? 'bg-gray-950/50' : ''} {mobile ? 'items-center bg-black' : 'flex-col'} flex gap-0.5 p-2 justify-center transition-all duration-300 hover:-translate-y-0.5 hover:gap-2 hover:text-carrot-400 hover:bg-gray-950/50">
            ...
            <p class="{mobile ? 'mobile' : 'hidden lg:block'} uppercase text-[80%] {$active == nav ? 'text-[var(--color-accent)]' : 'text-[var(--color-gray)] group-hover:text-[var(--color-accent)]'}">{nav}</p>
        </a>
    {/each}
</nav>
